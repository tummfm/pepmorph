...
- `artifacts/data_figs/`: dataset plots produced by `src/scripts/analysis.py` and `src/scripts/data_plotting.py`.
- `artifacts/descriptor_calc/`: logs/timing outputs from descriptor generation runs (`src/descriptor_calc/run.sh`).
...
- `artifacts/md_sims/`: MD validation data from the paper (runs + outputs) from `src/md_sims/cg/`.

...
from common import ROBUSTNESS_ARTIFACTS_DIR, TEAL, TEXT_COLOR, set_paper_style

DEFAULT_INCLUDE_TYPES = ["bigger_box_1230", "martini2"]
COHORT_LABELS = {
...
def peptide_dir_for(base_dir: Path, cohort_type: str, run: str, peptide: str) -> Path:
    runs_dir = resolve_runs_dir(base_dir)
    return runs_dir / cohort_type / run / peptide
...
def parse_args() -> argparse.Namespace:
    base_dir = ROBUSTNESS_ARTIFACTS_DIR
    outputs_dir = base_dir / "outputs"
    parser = argparse.ArgumentParser(description="Render CG robustness contact sheets from RMOI/AP CSV.")
    parser.add_argument("--base-dir", type=str, default=str(base_dir))
    parser.add_argument("--csv-path", type=str, default=str(outputs_dir / "rmoi_and_ap_by_run_total.csv"))
    parser.add_argument("--output-dir", type=str, default=str(outputs_dir))
    parser.add_argument("--include-types", nargs="*", default=DEFAULT_INCLUDE_TYPES)
...
def _run_sort_key(run_name):
    try:
        return int(str(run_name).split("_")[-1])
    except Exception:
        return str(run_name)
...
def main() -> int:
    ...
    out_root = output_dir / "pdf_reports_controls"
    ...
    for col in ["type", "target", "peptide", "run"]:
    ...
    if df.empty:
        raise SystemExit(f"No rows match --include-types {include_types}.")
    ...
    for cohort_type in include_types:
        cohort_name = COHORT_LABELS.get(cohort_type, cohort_type)
        ...
        for run in runs:
            for target in targets:
                ...
                entries = []
                for pep in peptides:
                    pdir = peptide_dir_for(base_dir, cohort_type, run, pep)

                    row = sub.loc[sub["peptide"] == pep].iloc[0] if (sub["peptide"] == pep).any() else None
                    ap = (
                        float(row["aggregation_propensity"])
                        if (row is not None and pd.notna(row["aggregation_propensity"]))
                        else None
                    )
                    ...
                    try:
                        if pdir.exists():
                            if not (pdir / args.top_name).exists() or not (pdir / args.traj_name).exists():
                                raise FileNotFoundError(
                                    f"Missing {args.top_name} or {args.traj_name} in {pdir}"
                                )

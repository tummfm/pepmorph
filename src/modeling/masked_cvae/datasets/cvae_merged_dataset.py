...
class OnlinePretrainDataset(Dataset):
    ...
    def __getitem__(self, _):
        L = self.rng.randint(1, self.max_length)
        seq = ''.join(self.rng.choices(self.amino_acids, k=L))
        cond = torch.zeros(self.condition_length, dtype=torch.float)
        cond[0] = float(len(seq)) / self.max_length
        return seq, cond


# Condition order: [length, is_assembled, ap, has_beta_sheet_content, hydrophobic_moment, net_charge].
FEATURES = [
    "length",
    "is_assembled",
    "ap",
...
class CVAEAllDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if idx < len(self.df):
            ...
            if self.random_mask:
                keep = torch.tensor(
                    [1.0 if self.rng.random() < self.mask_prob else 0.0 for _ in FEATURES],
                    dtype=torch.float,
                )
                mask_tensor *= keep
                if mask_tensor.sum() == 0:
                    mask_tensor[self.rng.randint(0, len(FEATURES) - 1)] = 1.0

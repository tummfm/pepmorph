...
mkdir -p "${DESC_ARTIFACTS}"

OVERALL_LOG="${OVERALL_LOG:-${DESC_ARTIFACTS}/orchestrator.log}"
TIMINGS_CSV="${TIMINGS_CSV:-${DESC_ARTIFACTS}/timings.csv}"
PEPTIDES_FASTA="${PEPTIDES_FASTA:-${REPO_ROOT}/data/raw/peptides.fst}"
GEN_SCRIPT="${GEN_SCRIPT:-${SCRIPT_DIR}/gen.py}"
DATASET_SCRIPT="${DATASET_SCRIPT:-${SCRIPT_DIR}/pepfold_pipeline/generate_pep_dataset.py}"

GEN_CMD=(python3 "${GEN_SCRIPT}" "${PEPTIDES_FASTA}" --sa-workers=8 --num-workers=8)
DATASET_CMD=(python3 "${DATASET_SCRIPT}")
